...
func GetTxtDataByTasks(tasks []string) (TxtTasksData, bool) {
	...
	for _, task := range tasks {
		if task == "Boost account followers" {
			data := ReadTxtFile("usernames for boost followers", "data/my_usernames.txt")
			if len(data) == 0 {
				return changeData, false
			}
			changeData.UsernamesForBoostFollowers = data
		}
		if task == "Get account creation date by username" {
			data := ReadTxtFile("target usernames", "data/scraped/usernames.txt")
			if len(data) == 0 {
				return changeData, false
			}

			changeData.InputToScrape = data
		}
		if strings.Contains(task, "Username to user ID") {
			data := ReadTxtFile("tokens for scraping", "data/scraped/tokens_for_scraping.txt")
			if len(data) == 0 {
				return changeData, false
			}
			changeData.TokensForScraping = data

			data = ReadTxtFile("usernames", "data/scraped/usernames.txt")
			if len(data) == 0 {
				return changeData, false
			}
			changeData.InputToScrape = data
		...
		if strings.Contains(task, "Scrape ") || strings.Contains(task, "Get account creation date by username") {
			data := ReadTxtFile("tokens for scraping", "data/scraped/tokens_for_scraping.txt")
			if len(data) == 0 {
				return changeData, false
			}
			...
			if !strings.Contains(task, "Get account creation date by username") {
				data = ReadTxtFile("users to scrape", "data/scraped/input_to_scrape.txt")
				if len(data) == 0 {
					return changeData, false
				}
				changeData.InputToScrape = data
		...
		if task == "Unfollow" {
			data := ReadTxtFile("user ids", "data/input_to_scrape.txt")
			if len(data) == 0 {
				return changeData, false
			}
			changeData.UserIDsToUnfollow = data
		}
		...
		if task == "Change Password" || task == "Account's BrutForce" {
		...
		if task == "Mutual Subscription" {
			...
			var parserUsernames []string
			for _, username := range data {
				user := strings.Split(username, ":")[0]
				parserUsernames = append(parserUsernames, user)
			}
			if len(parserUsernames) == 0 {
				return changeData, false
			}
			changeData.MyUsernames = parserUsernames
		}
...
func GetTasksTargetInfo(tasks []string) (TasksTargetInfo, bool) {
	...
	for _, task := range tasks {
		...
		if task == "Unretweet" {
			tasksInfo.UnretweetLinks = userInputToSlice("Paste your link for unretweet")
		}
		if task == "Like" {
			tasksInfo.LikeLinks = userInputToSlice("Paste your link for like")
		}
		if task == "Unlike" {
			tasksInfo.UnlikeLinks = userInputToSlice("Paste your link for unlike")
		}
		if task == "Quote Tweet" {
...
type TxtTasksData struct {
	...
	Locations                  []string
	MyUsernames                []string
	UserIDsToUnfollow          []string
	TokensForScraping          []string
	InputToScrape              []string
	UsernamesForBoostFollowers []string
	AccountsForFollowersBoost  []string
}

type TasksTargetInfo struct {
	FollowTargets            []string
	RetweetLinks             []string
	UnretweetLinks           []string
	LikeLinks                []string
	UnlikeLinks              []string
