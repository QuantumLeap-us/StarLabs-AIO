...
func (twitter *Twitter) VotePoll(link, answer string) (bool, string, []string) {
    var logs []string
    errorType := "Unknown"
    ...
    _, tweetDetail := twitter.getTweetDetail(link)
    if tweetDetail == "" {
        logs = append(logs, "Failed to get tweet details")
        return false, errorType, logs
    }
    ...
    for i := 0; i < twitter.config.Info.MaxTasksRetries; i++ {
        ...
        if err != nil {
            logs = append(logs, fmt.Sprintf("Failed to build vote poll request: %s", err.Error()))
            continue
        }

        // Headers remain the same as in original
        ...
        if err != nil {
            logs = append(logs, fmt.Sprintf("Failed to do vote poll request: %s", err.Error()))
            continue
        }
        ...
        if err != nil {
            logs = append(logs, fmt.Sprintf("Failed to read vote poll response body: %s", err.Error()))
            continue
        }
        ...
        if resp.StatusCode >= 200 && resp.StatusCode <= 299 {
            logs = append(logs, fmt.Sprintf("%s voted in poll %s", twitter.Username, tweetID))
            return true, "", logs
            ...
            logs = append(logs, fmt.Sprintf("Unknown response while vote poll: %s", bodyString))
            continue
        }
    }

    logs = append(logs, "Unable to do vote poll")
    return false, errorType, logs
}
