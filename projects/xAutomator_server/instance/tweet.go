...
func (twitter *Twitter) Tweet(tweetContent string) (bool, string, []string) {
	var logs []string
	errorType := "Unknown"
	...
	for i := 0; i < twitter.config.Info.MaxTasksRetries; i++ {
		...
		if err != nil {
			logs = append(logs, fmt.Sprintf("Failed to do tweet request: %s", err.Error()))
			continue
		}
		...
		if err != nil {
			logs = append(logs, fmt.Sprintf("Failed to read tweet response body: %s", err.Error()))
			continue
		}
		...
		if resp.StatusCode >= 200 && resp.StatusCode <= 299 {
			if strings.Contains(bodyString, "duplicate") {
				...
				if err != nil {
					logs = append(logs, fmt.Sprintf("Failed to unmarshal duplicate tweet response: %s", err.Error()))
					continue
				}
				logs = append(logs, fmt.Sprintf("%s already tweeted this content", twitter.Username))
				return true, "", logs
			} else if strings.Contains(bodyString, "create_tweet") {
				logs = append(logs, fmt.Sprintf("%s successfully tweeted", twitter.Username))
				return true, "", logs
			}
			...
			logs = append(logs, fmt.Sprintf("Unknown response while tweeting: %s", bodyString))
			continue
		}
	}

	logs = append(logs, "Unable to send tweet")
